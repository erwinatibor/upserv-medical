/* ============================================================
   APEX DENTAL — Main JS   js/main.js
   ============================================================ */
'use strict';
document.addEventListener('DOMContentLoaded', () => {
  initNav(); initScrollReveal(); initAccordion();
  initCounters(); initSmoothScroll(); initFormValidation(); initServicesFilter();
});
...
function initNav() {
  ...
  const onScroll = () => {
    ...
    let active = null;
    sections.forEach(s => { const r = s.getBoundingClientRect(); if (r.top <= 90 && r.bottom > 90) active = s.id; });
  ...
  document.querySelectorAll('.accordion-btn').forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      document.querySelectorAll('.accordion-btn.active').forEach(b => {
        b.classList.remove('active'); b.setAttribute('aria-expanded','false'); b.nextElementSibling.classList.remove('open');
      });
      if (!isOpen) { btn.classList.add('active'); btn.setAttribute('aria-expanded','true'); btn.nextElementSibling.classList.add('open'); }
    });
  });
}
...
function initCounters() {
  ...
  const io = new IntersectionObserver((entries) => {
    entries.forEach(e => {
      ...
      const suffix = el.dataset.suffix || ''; const isFloat = el.dataset.float === 'true';
      const dur = 1600; const start = performance.now();
      const tick = (now) => {
        const p = Math.min((now-start)/dur,1); const ease = 1-Math.pow(1-p,3);
        const val = end*ease; const disp = isFloat ? val.toFixed(1) : Math.round(val).toLocaleString();
        el.textContent = disp+suffix; if (p<1) requestAnimationFrame(tick);
      };
...
function initFormValidation() {
  ...
  form.addEventListener('submit', e => {
    ...
    form.querySelectorAll('[required]').forEach(f => { const empty = !f.value.trim(); f.style.borderColor = empty ? 'var(--red-600)' : ''; if (empty) valid = false; });
    if (!valid) return;
    const btn = form.querySelector('[type="submit"]'); const orig = btn.textContent;
    btn.textContent = '✓ Request Received'; btn.style.background = 'var(--sage)'; btn.style.borderColor = 'var(--sage)'; btn.disabled = true;
